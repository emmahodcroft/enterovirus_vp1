...
rule files:
    params:
        raw_vipr = "data/allEntero-11Dec18.tsv", #"data/allEntero-31Jan19.tsv", #raw VIPR download!
...
rule concat_meta:
    input:
        metadata = [files.swedish_meta, files.manual_meta, "genbank/genbank_vp1_meta.tsv"]
    output:
...
rule concat_sequences:
    input:
        files.swedish_seqs, files.manual_seqs, "genbank/genbank_vp1_sequences.fasta"
    output:
